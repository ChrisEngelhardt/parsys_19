...
int main(int argc, char **argv) {
  ...
  long long NPerSlot = N/ ( (int)sqrt(numProcs) );
  ...
  for (int t = 0; t < T; t++) {
    ...
    //Send
    if(myrank != 0){
      MPI_Isend(&A[NPerSlot*myrank][0][0], N*N, MPI_DOUBLE, myrank-1, TAG_L, MPI_COMM_WORLD, &requestL);
    }
    
    if(myrank != lastProcess){
      MPI_Isend(&A[NPerSlot*(myrank+1)-1][0][0], N*N, MPI_DOUBLE, myrank+1, TAG_R, MPI_COMM_WORLD, &requestR);
    }
