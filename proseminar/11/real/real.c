...
static void setup(int *n1, int *n2, int *n3)
{
  ...
  ng[lt][1] = ny[lt];
  ng[lt][2] = nz[lt];
  //openmp not possible, since ng[k] depends on ng[k+1]
  for (k = lt-1; k >= 1; k--) {
    for (ax = 0; ax < 3; ax++) {
      ng[k][ax] = ng[k+1][ax]/2;
    }
  ...
  *n3 = 3 + ie3 - is3;
  ...
  ir[lt] = 0;
  //openmp not possible, since ir[j] depends on ir[j+1]
  for (j = lt-1; j >= 1; j--) {
...
static void zran3(void *oz, int n1, int n2, int n3, int nx1, int ny1, int k)
{
  ...
  //---------------------------------------------------------------------
  //openmp possible???, but attention to randlc
  for (i3 = 1; i3 < e3; i3++) {
    starts[i3] = x0;
    rdummy = randlc(&x0, a2);
  }
